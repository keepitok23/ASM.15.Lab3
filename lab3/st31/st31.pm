...
sub st31
{
	...
	print $q->start_html(-title => "Соревнования");
	...
	{
	my $sth = $db->prepare("INSERT INTO st31 (name,nickname,team,status) VALUES (?,?,?,?)");
	;
	...
	sub delete_object
	{
	my $sth = $db->prepare("DELETE FROM st31 WHERE item = ?");
	...
	if ($q->param('name') ne "" && $q->param('nickname') ne "" && $q->param('team') ne "")
	{
	my $sth = $db->prepare("UPDATE st31 SET name = ?,nickname = ?,team = ?, status = ? WHERE item = ?");
	$sth->execute(
		$q->param('name'),
		$q->param('nickname'), 
		$q->param('team'),
	...
	sub show 
	{
		...
		my $sth = $db->prepare("SELECT * FROM st31");
		$sth->execute();
